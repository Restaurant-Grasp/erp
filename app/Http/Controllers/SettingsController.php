  ...
  public function update(Request $request)
{
    $request->validate([
        ...
        'settings.*' => 'nullable|string|max:10000', // Increased for JSON data
    ...
    try {
        ...
        foreach ($request->settings as $settingId => $value) {
            ...
            switch ($setting->setting_type) {
                ...
                case 'json':
                    ...
                    $this->validateJsonSetting($value, $setting->setting_key);
                    // Ensure it's valid JSON
                    if (!empty($value)) {
                        $decoded = json_decode($value, true);
                        $value = json_encode($decoded); // Re-encode to ensure format
                    } else {
        ...
        // ... rest of existing update method code remains the same
    ...
    private function validateJsonSetting($value, $settingKey)
{
    if ($settingKey === 'cloud_server_hosting') {
        $decoded = json_decode($value, true);
        
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new \InvalidArgumentException('Invalid JSON format for cloud server hosting');
        }
        
        // Validate structure
        if (!is_array($decoded)) {
            throw new \InvalidArgumentException('Cloud server hosting must be an array');
        }
        
        foreach ($decoded as $index => $item) {
            if (!isset($item['feature']) || !isset($item['description'])) {
                throw new \InvalidArgumentException("Item {$index} must have 'feature' and 'description' fields");
            }
        }
